    ...
    try {
      ...
      const docDefinition: any = {
        ...
        footer: function (currentPage: number, pageCount: number) {
          ...
          // Get footer text from translations or use default
          const footerText =
            options.coverPage?.translations?.footer ||
            "Tomas from iustomas.ai and Affiliates";
  ...
  private buildCoverPage(
    ...
    if (coverPage.clientName) {
      const preparedForText =
        coverPage.translations?.preparedFor || "Prepared for:";
      content.push({
        text: `${preparedForText} ${coverPage.clientName}`,
        fontSize: 13,
        alignment: "center",
        margin: [0, 0, 0, 20],
        color: "#34495E",
    ...
    if (coverPage.referenceNumber) {
      const referenceText = coverPage.translations?.reference || "Reference:";
      content.push({
        text: `${referenceText} ${coverPage.referenceNumber}`,
        fontSize: 11,
        alignment: "center",
        margin: [0, 0, 0, 30],
        color: "#7F8C8D",
    ...
    if (coverPage.contactInfo) {
      ...
      if (coverPage.contactInfo.phone) {
        const telephoneText = coverPage.translations?.telephone || "Tel:";
        contactStack.push({
          text: `${telephoneText} ${coverPage.contactInfo.phone}`,
          fontSize: 11,
          alignment: "center",
          margin: [0, 0, 0, 4],
          color: "#7F8C8D",
